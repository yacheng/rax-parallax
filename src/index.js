...
function BindingParallax(props) {
    const parallaxRef = useRef();
    const mountRef = useRef();

    useEffect(function() {
        if (!mountRef.current) {
            let {bindingScroller, transform = [], backgroundColor, opacity, extraBindingProps = []} = props;
            let parallax = getEl(parallaxRef.current);
            let bindingProps = [];
    ...
    return <View ref={parallaxRef} style={{...props.style}}>{props.children}</View>;
}


function Parallax(props) {
    let {bindingScroller} = props;
    ...
    if (isSupportBinding || !isWeex) {
        return <BindingParallax {...props} bindingScroller={scroller} />;
    }

    return isWeex ? <parallax {...props} bindingScroller={scroller} /> :
        <View {...props} />;
}
